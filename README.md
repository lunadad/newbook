...
| 작업 | 주기 | 스크립트 |
|---|---|---|
| 오늘의 책 — 예스24·알라딘 | 화·금 5분 (06~23시) | `newbook_today_book_watch_http.sh` |
| 오늘의 책 — 교보문고 | 화·금 10분 (06~23시) | `newbook_today_book_watch_kyobo.sh` |
| 오늘의 책 — 전체 안전망 | 4시간 (매일) | `newbook_today_book.sh` |
| 문학 신상품 | 30분 | `newbook_new_releases.sh` |
| 실시간 베스트셀러 | 1시간 | `newbook_bestsellers.sh` |

오늘의 책은 3사 모두 **화요일과 금요일에만** 갱신되지만, 그 안에서 **몇 시에 바뀌는지는 일정하지 않다**
(같은 화요일에도 교보문고는 16시 이전, 예스24는 16:25 무렵, 알라딘은 17:50 무렵에 바뀐 사례).
그래서 갱신 시각을 추측한 좁은 크론 창 대신, 화·금에 한해 **짧은 주기 폴링 + 변경 게이트**를 쓴다.
다른 요일은 4시간 안전망만으로 커버한다.
